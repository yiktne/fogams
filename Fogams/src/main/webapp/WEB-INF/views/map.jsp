...
<script type="text/javascript" src="//dapi.kakao.com/v2/maps/sdk.js?appkey=3a1eab4890ae0b2c4a4c97691189904b&libraries=clusterer"></script>
...
<div id="tabs">
	<ul id="company_menu" >
		...
		<li><a href="#tabs-2">관광지</a></li>
		<li><a href="#tabs-3">음식</a></li>
		<li><a href="#tabs-4">찜</a></li>
	</ul>
	<div id="tabs-1" class="company_list">
			<table border="1">	
				<tbody>
					<c:choose>
						...
						<c:otherwise>
							<c:forEach items="${rooms_list }" var="dto">
								<tr>
									<td><img src="${pageContext.request.contextPath}/img.do?img=${dto.company_img}" alt="${dto.company_img }" class="company__left_img"></td>
								</tr>
								<tr>
									<td><a onclick="roomsSelect(${dto.company_no },'${dto.company_product }','${dto.company_money }')">${dto.company_product }</a></td>
								</tr>
								<tr>
									<td>${dto.company_money }</td>
								</tr>
							</c:forEach>
						</c:otherwise>
					</c:choose>
	...
	<div id="tabs-2" class="company_list">
			<table border="1">	
				<tbody>
					<c:choose>
						<c:when test="${empty tour_list }">
							<tr>
								<td>관광지가 존재하지 않습니다</td>
							</tr>
						</c:when>
						<c:otherwise>
							<c:forEach items="${tour_list }" var="dto">
								<tr>
									<td><img src="${pageContext.request.contextPath}/img.do?img=${dto.company_img}" alt="${dto.company_img }" class="company__left_img"></td>
								</tr>
								<tr>
	...
	<div id="tabs-3" class="company_list">
			<table border="1">	
				<tbody>
					<c:choose>
						<c:when test="${empty food_list }">
							<tr>
								<td>음식점이 존재하지 않습니다</td>
							</tr>
						</c:when>
						<c:otherwise>
							<c:forEach items="${food_list }" var="dto">
								<tr>
									<td><img src="${pageContext.request.contextPath}/img.do?img=${dto.company_img}" alt="${dto.company_img }" class="company__left_img"></td>
								</tr>
								<tr>
	...
	<div id="tabs-4" class="company_list">
			<table border="1">	
				<tbody>
					<c:choose>
						<c:when test="${empty mark_list }">
							<tr>
								<td>찜목록이 존재하지 않습니다</td>
							</tr>
						</c:when>
						<c:otherwise>
	...
	<form id="plan" action="#">
		...
		<label>마지막날짜</label>
		<input type="date" id="lastdate"/><br/>
	 	<input type="button" value="여행 계획 시작" onclick="planstart()"/>
	 	<div id="planlist"></div>
	 	
	 	
	 	<div id="planfooter">
	 		<p>총<label>원</label></p>
	 		<input type="button" onclick="sandplan()" value="저장"/>
	 		<input type="button" value="자동계획"/>
	 	</div>
	</form>
	<div id="plantime" style="display: none">
		<form>
			<table>
				...
				<tr>
					<td colspan="2" align="center"><input type="button" value="저장" onclick="addPlan()" /><input type="button" value="취소" onclick="closePlanTime()" /></td>
				</tr>
			</table>
		</form>
	</div>
